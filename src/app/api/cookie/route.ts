...
export async function POST(req: Request) {
  ...
  return NextResponse.json({ ok: true, username: resolved }, { headers: cors(req) });
}

/**
 * The sync runs in the extension's service worker, whose origin is
 * chrome-extension://<id> — an id that differs per install, so it can only be
 * matched by scheme.
 */
function cors(req?: Request) {
  const origin = req?.headers.get("origin") ?? "";
  const allowed =
    origin === "https://leetcode.com" || origin.startsWith("chrome-extension://");
  return {
